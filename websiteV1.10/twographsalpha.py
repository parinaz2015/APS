...
dfdiff=pandas.read_csv('diff.csv').sort_values(by=['wavelength'],ignore_index=True)
...
x_slider = RangeSlider(start=500, end=1800, value=(500,1800), step=.1, title="Wavelength(nm)",width=520, align="center" )
y_slider = RangeSlider(start=-5000, end=5000, value=(-5000,5000), step=.1, title="polarizability (a.u.)",width=520 ,align="center")
...
for x,y in zip(x_data,y_data):
    p.line(x= x, y=y ,color="firebrick", line_width=1.5, legend_label='CS6s',alpha=1,muted_alpha=0 )

x_data,y_data = data_function(df2)
for x,y in zip(x_data,y_data):
    p.line( x =x, y=y ,color="darkviolet", line_width=1.5,legend_label='7p1',alpha=1,muted_alpha=0  )
...
for x,y in zip(x_data_diff,y_data_diff):
    diffPlot= p.line( x =x, y=y ,color="grey",line_width=1.5, alpha=0, legend_label='diff',muted_alpha=1 )
