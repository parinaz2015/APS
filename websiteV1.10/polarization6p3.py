...
from bokeh.models import SingleIntervalTicker,ContinuousTicker,BasicTicker, LinearAxis, Grid, Slider, CustomJS, Button, Label, LabelSet
...
df2=pandas.read_csv("Data/Cs6p3m1.csv")
source2= ColumnDataSource(df2)
# dividing data to get line graph from separate parts

df3=pandas.read_csv("Data/Cs6p3m3.csv")
source3= ColumnDataSource(df3)

# dfdiff=pandas.read_csv('Data/cs6scs6p1.csv').sort_values(by=['wavelength'],ignore_index=True)
...
def data_function(input_data):
    data =[]
    data2 = []
    y = input_data['polarizability']
    x = input_data['wavelength']
    negative= True
    ...
    for i in range(y.shape[0]):
        ...
        elif(negative==False and y[i]<=0 and y[i-1]-y[i]>4000):
            negative=True
            y_data.append(data)
            x_data.append(data2)
            data = []
...
def get_difference(df,df2):
    both = set(df['wavelength']).intersection(df2['wavelength'])

    list_a = df['wavelength'].tolist()
    list_b = df2['wavelength'].tolist()

    indices_A = [list_a.index(x) for x in both]
    indices_B = [list_b.index(x) for x in both]
    diff = []
    for i in range(len(indices_A)):
        difference = df2['polarizability'][indices_B[i]] - df['polarizability'][indices_A[i]]
        diff.append(difference)

    both=np.asarray(list(both))
    diff=np.asarray(diff)
    return(both,diff)

# creates html file
output_file('Cs6p3.html')
TOOLTIPS = [
    ("wavelength:", "$x{1.1}" ),
...
tickery = BasicTicker(max_interval=1000, min_interval=100, desired_num_ticks=10)
...
x_slider = RangeSlider(start=600, end=1800, value=(600,1800), step=.1, title="Wavelength(nm)",width=510, align="center" )
y_slider = RangeSlider(start=-5000, end=5000, value=(-5000,5000), step=.1, title="polarizability (a.u.)",width=510 ,align="center")
...
for x,y in zip(x_data,y_data):
    p.line(x= x, y=y ,color="#ff005b", legend_label='6s')

x_data,y_data = data_function(df2)
for x,y in zip(x_data,y_data):
    p.line( x =x, y=y ,color="navy", legend_label="6p3/2,|m|=1/2" )

x_data,y_data = data_function(df3)
for x,y in zip(x_data,y_data):
    p.line( x =x, y=y ,color="green", legend_label="6p3/2,|m|=3/2" )
# For showing magiczero coordinations using lables. first find the points,set lables on them,and show them
...
# This is how In the First run I created diff.csv file
# x_diff,y_diff=get_difference(df,df2)
# with open('diff.csv', 'w') as f:
#     writer = csv.writer(f)
#     writer.writerows(zip(x_diff, y_diff))

# x_data_diff,y_data_diff = data_function(dfdiff)
# for x,y in zip(x_data_diff,y_data_diff):
#     p.line( x =x, y=y ,color="green", legend_label='diff' )
    # p.scatter( x =x, y=y ,color="pink", legend_label='diff',marker="circle" )
...
# print (df2)
layout =column(p,x_slider,y_slider,button)
