...
"""
#notifies user of any update to Dropbox files comapred to the last check on the Dropbox file
import dropbox
import json
import pandas as pd
...
#saves a list of current files on Dropbox-> in local system in jason format
...
#        print(df.at[idx,"file_name"])
        print("Changes in Dropbox Folder\n",df.loc[idx])
        
        if(df_old.size>df_new.size):
            print("------------Deleted------------\n")
        elif(df_old.size==df_new.size):
            print("------------Updated------------\n")
        else:
            print("------------Created------------\n")
    else:
        print("\n No changes in Dropbox Folder\n")
...
#    
##    reads Today's list of files in dropbox and prints them
#    print('\n Todays list of files in dropbox \n',read_file_data(path))
##    read the last list that is stored in current_files.json
#    print('\n Last list of files from dropbox \n',read_current_files())
##    check_for_change(path)
...
#    read_file_data(path)
    save_current_files(drop_box_path)
    #if there has been any update or any new file we should download those files
